...
(function (factory) {
      ...
      setTimeout(function () {
        ...
        $("body")
          .on("click.dtb-collection", function (e) {
            ...
            if (
              (!$(e.target).parents()[back]().filter(content).length && !$(parent).hasClass("dt-buttons")) ||
  ...
  var _filename = function (config, dt) {
    ...
    var filename =
      config.filename === "*" && config.title !== "*" && config.title !== undefined && config.title !== null && config.title !== ""
        ? config.title
        : config.filename;
